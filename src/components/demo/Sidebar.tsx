...
const SidebarTooltip = ({ children, text, collapsed, className }: { children: React.ReactNode, text: string, collapsed: boolean, className?: string }) => (
        ...
        <aside className={cn(
            ...
            <div className={cn("p-5 pb-0 flex flex-col h-full", collapsed && "p-3 items-center")}>
                ...
                <div className={cn(
                    ...
                    collapsed ? "flex-col items-center justify-center mb-8 mt-1" : "flex-col items-center justify-center gap-2 mb-6"
                )}>
                    {toggleCollapse && (
                        <div className={cn("flex justify-center", collapsed ? "relative z-30 w-full" : "absolute right-0 top-0 translate-x-1 -translate-y-1 z-30")}>
                    ...
                    {!collapsed && (
                        <div className="flex flex-col items-center overflow-hidden">
                            <h2 className="text-lg font-semibold tracking-tight text-pied-text leading-none whitespace-nowrap">Pied AI</h2>
                            <span className="text-[10px] text-pied-muted font-medium tracking-[0.15em] uppercase mt-1 whitespace-nowrap">Intelligence</span>
                        </div>
                    )}
                </div>
                ...
                {!collapsed && (
                    <div className="w-full mb-3 md:hidden">
                        <label className="text-[10px] uppercase text-pied-muted font-semibold tracking-wider mb-2 block pl-1">Role</label>
                        <Select value={selectedRole} onValueChange={setSelectedRole}>
                            ...
                            <SelectContent className="bg-pied-surface border-pied-border text-pied-text z-50 rounded-pied">
                                {ROLES.map(role => (
                                    <SelectItem key={role} value={role} className="focus:bg-white/[0.06] focus:text-pied-text cursor-pointer">{role}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                                ...
                                <SidebarTooltip key={chat.id} text={chat.title || "Untitled"} collapsed={collapsed}>
                                    ...
                                    >
                                        ...
                                        <div className={cn(
                                            ...
                                            <p className="text-[13px] font-medium truncate flex-1">{chat.title || "Untitled Conversation"}</p>
                                            <button
                ...
                <div className={cn("mt-auto space-y-3 shrink-0 w-full", collapsed ? "pt-3" : "p-0 pt-3")}>
                    ...
                    <div className={cn(
                        ...
                        <button className="w-full bg-pied-accent py-2 rounded-pied text-xs font-semibold hover:bg-pied-accent-hover transition-colors text-white whitespace-nowrap">Upgrade to Pro</button>
                    </div>
                        ...
                        <SidebarTooltip text={t('common.settings')} collapsed={collapsed}>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <div className={cn("flex items-center px-2.5 py-2 text-pied-muted hover:text-pied-text transition-all cursor-pointer rounded-pied hover:bg-white/[0.03] overflow-hidden w-full", collapsed ? "justify-center gap-0" : "gap-2.5")}>
                                        <span className="material-symbols-outlined text-[18px] shrink-0">settings</span>
                                        <div className={cn("transition-all duration-300 whitespace-nowrap overflow-hidden block", collapsed ? "w-0 opacity-0" : "w-auto opacity-100")}>
                                            <p className="text-[13px] font-medium">{t('common.settings')}</p>
                                        </div>
                                    </div>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent side={collapsed ? "right" : "top"} sideOffset={16} className="w-56 bg-pied-surface border border-pied-border text-pied-text rounded-pied">
                                ...
                                >
                                    ...
                                    <span className="font-mono text-[11px] text-pied-muted bg-pied-surface-2 px-1.5 py-0.5 rounded border border-pied-border uppercase">{language}</span>
                                </DropdownMenuItem>
                                <DropdownMenuSeparator className="bg-pied-border" />
                                <DropdownMenuItem
                    ...
                    {/* Delete All Chats Dialog */}
                        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                            <AlertDialogContent className="bg-pied-surface border border-pied-border text-pied-text rounded-pied">
                                ...
                                <AlertDialogFooter>
                                    <AlertDialogCancel className="bg-transparent border-pied-border text-pied-text hover:bg-white/[0.06] hover:text-pied-text rounded-pied">Cancel</AlertDialogCancel>
                                    <AlertDialogAction
                        ...
                        {/* ─── User Section ─── */}
                        <div className={cn("flex items-center gap-3 px-2.5 py-2.5 border-t border-pied-border pt-3 w-full", collapsed && "flex-col border-none pt-2 px-0")}>
                            {session ? (
                                <>
                                    <div className="size-8 shrink-0 rounded-pied bg-cover bg-center border border-pied-border relative overflow-hidden" >
                                        {session.user?.image ? (
                                            <NextImage src={session.user.image} alt={session.user.name || "User"} width={32} height={32} className="object-cover" />
                                        ) : (
                                    ...
                                    <div className={cn("flex flex-col overflow-hidden transition-all duration-300", collapsed ? "w-0 opacity-0 hidden" : "flex-1 opacity-100")}>
                                        <p className="text-[13px] font-semibold text-pied-text truncate">{session.user?.name || "User"}</p>
                                    ...
                                    <SidebarTooltip text="Logout" collapsed={collapsed} className={cn(!collapsed && "w-auto")}>
                                        <button
                                            ...
                                            className={cn("text-pied-muted hover:text-pied-text transition-colors flex items-center justify-center p-1", collapsed && "mt-1")}
                                            aria-label="Logout"
                                        >
                                ...
                                <SidebarTooltip text="Login with Google" collapsed={collapsed}>
                                    ...
                                    <span className={cn("transition-all duration-300 whitespace-nowrap overflow-hidden block", collapsed ? "w-0 opacity-0" : "w-auto opacity-100")}>
